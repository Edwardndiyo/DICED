...
const ProfilePage = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        const profileResponse = await axios.get(` http://edwardndiyoo.pythonanywhere.com/get_profile/${username}`);
  ...
  useEffect(() => {
    const fetchGameRecords = async () => {
      try {
        ...
        const gameRecordsResponse = await axios.get(` http://edwardndiyoo.pythonanywhere.com/get_game_records/${username}`);
  ...
  const handleSave = async () => {
    if (isEditing) {
      ...
      try {
        const username = localStorage.getItem('username'); // Assuming the username is stored in localStorage
        const response = await axios.post(' http://edwardndiyoo.pythonanywhere.com/update_profile', {
          ...formData,
          username,
